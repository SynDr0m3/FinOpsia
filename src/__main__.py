...
from src.ml.persistence import train_and_save_model
...
def main():
    ...
    run_parser.add_argument(
        ...
        type=int,
        required=True,
        help="User ID that owns every account in the CSV",
    )
    run_parser.add_argument(
        "--forecast",
        action="store_true",
    ...
    fore_parser.add_argument(
        "--user-id",
        type=int,
        help="Owning user ID. If omitted, the owner is derived from the account.",
    )
    ...
    logger.info(
        ...
        extra={"account_id": args.account_id, "user_id": args.user_id},
    )
    train_and_save_model(
    ...
    logger.success(
        f"Forecaster retraining completed for account {args.account_id}",
        extra={"account_id": args.account_id, "user_id": args.user_id},
    )
